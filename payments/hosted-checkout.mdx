---
title: "Accept USDC Payments with FlareHQ Hosted Checkout Page"
sidebarTitle: "Hosted Checkout"
description: "Create a USDC checkout session and redirect customers to FlareHQ's pre-built payment page on Arc testnet. Sessions expire after 120 minutes."
---
...
Send the customer's browser to `session.checkoutUrl`. They land on the hosted checkout page at `https://flarehq.xyz/checkout/{reference}` where they connect their wallet and approve the USDC transfer.
...
<ResponseField name="checkoutUrl" type="string">
  The full hosted checkout URL — `https://flarehq.xyz/checkout/{reference}`. Redirect your customer here immediately after creation. Returned as `session.checkoutUrl` by the SDK.
</ResponseField>
...
| Event | Fired when |
|---|---|
| `payment.settled` | USDC successfully transferred to the recipient address |

FlareHQ does not emit a `payment.failed` webhook. If a session expires or its on-chain transfer fails, poll [GET /api/payments/verify/{reference}](/api-reference/payments/verify) — the status returns `FAILED` or `EXPIRED`.
...
```json
{
  ...
  "metadata": { "customerId": "usr_123" },
  "txHash": "0xabc123...",
  "settledAt": "2025-01-15T13:12:44.000Z"
}
```
...
<Note>
  Use the `metadata` field to attach your own order or customer IDs to every session. When the `payment.settled` webhook fires, that metadata comes back in the payload — so you can fulfil the order without an extra database lookup.
</Note>
