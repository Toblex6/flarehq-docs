---
title: "Create Shareable and Persistent USDC Payment Links"
sidebarTitle: "Payment Links"
description: "Generate persistent, reusable USDC checkout URLs for products, subscriptions, or donation pages — no per-transaction checkout session required."
---
...
<ResponseField name="checkoutUrl" type="string">
  The shareable hosted checkout URL, e.g. `https://flarehq.xyz/checkout/{reference}`. Returned as `session.checkoutUrl` by the SDK.
</ResponseField>
...
| Event | Fired when |
|---|---|
| `payment.settled` | A buyer completes payment via the link |

There is no `payment.failed` webhook — after a failed or expired payment, poll [GET /api/payments/verify/{reference}](/api-reference/payments/verify) for the authoritative `FAILED`/`EXPIRED` status.
...
```json
{
  ...
  "txHash": "0x123...789",
  "settledAt": "2025-01-15T11:22:33.000Z"
}
```
