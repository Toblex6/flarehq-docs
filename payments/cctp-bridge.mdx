---
title: "Bridge USDC to Arc Testnet via Circle CCTP"
sidebarTitle: "CCTP Bridge"
description: "Transfer native USDC from Ethereum, Arbitrum, or Solana into Arc testnet with 1:1 capital efficiency using Circle CCTP."
---
...
The CCTP bridge routes funds **from** your FlareHQ-created Circle wallet on the source chain. Bring-your-own (external) wallets cannot be bridged automatically — the transfer must originate from a wallet provisioned at FlareHQ sign-up.
...
### Sign In and Start the Transfer

The bridge endpoint reads your identity from your session token — you do not pass your wallet address in the request body.
...
The bridge runs asynchronously. Use the `reference` from the response to poll the status endpoint until `status` reaches `"completed"`.

```bash
curl "https://flarehq.xyz/api/cctp/transfer/status?reference={reference}" \
  -H "Authorization: Bearer fhq_sec_..."
```
...
<ResponseField name="reference" type="string">
  Unique transfer reference. Use this with `GET /api/cctp/transfer/status?reference=...` to check progress.
</ResponseField>
...
```json
{
  "success": true,
  "status": "pending",
  "reference": "cctp_ref_8x2nq7w4",
  "message": "Bridge started — poll /api/cctp/transfer/status?reference=cctp_ref_8x2nq7w4 to check progress."
}
```
...
```bash
curl "https://flarehq.xyz/api/cctp/transfer/status?reference=cctp_ref_8x2nq7w4" \
  -H "Authorization: Bearer fhq_sec_..."
```
