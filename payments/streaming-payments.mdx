---
title: "Stream USDC Payments in Real Time"
sidebarTitle: "Streaming Payments"
description: "Distribute USDC continuously per-second using the ArcFlareStream contract. Recipients withdraw vested funds at any time."
---

Payment streaming replaces lump-sum or scheduled transfers with a continuous, per-second USDC flow. The sender deposits USDC upfront into the `ArcFlareStream` contract on Arc testnet, and the contract releases funds to the recipient at a fixed rate every second. Recipients can withdraw their vested balance at any time — they never have to wait until the end of the stream — and senders can stop the stream early to reclaim unstreamed USDC.
...
<CardGroup cols={2}>
  ...
  <Card title="AI Agent Compensation" icon="robot">
    Pay autonomous agents for completed compute tasks in real time, tied to their on-chain SCA wallet.
  </Card>
</CardGroup>

## How Streaming Works

1. **Deposit** — The sender locks a total USDC amount into `ArcFlareStream.sol`.
...
curl -X POST https://flarehq.xyz/api/payments/stream \
  ...
  -d '{
    "senderSCA": "0xYourSenderSCAAddress",
...
<ParamField body="senderSCA" type="string" required>
  The Circle Smart Contract Account (SCA) address of the sender. This wallet must hold sufficient USDC to cover `totalDeposited`.
</ParamField>

<ParamField body="receiverSCA" type="string" required>
  The SCA wallet address of the recipient. They will withdraw vested USDC to this address.
</ParamField>
...
curl -X POST https://flarehq.xyz/api/payments/stream/stop \
  ...
  -d '{
    "reference": "stream_m3n7p1x9_ab12cd",
    "callerSCA": "0xYourSenderSCAAddress"
  }'
```
...
curl "https://flarehq.xyz/api/payments/stream?sender=0xYourSenderSCAAddress&status=ACTIVE" \
  -H "Authorization: Bearer fhq_sec_..."
