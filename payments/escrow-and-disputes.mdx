...
description: "Lock USDC on-chain in ArcFlareEscrow.sol with milestone-based release and arbiter-mediated dispute resolution."
---

FlareHQ's escrow service lets you lock USDC in `ArcFlareEscrow.sol` on Arc testnet until both parties confirm that conditions are met. If either side disagrees, either party can raise a dispute — at which point the designated arbiter reviews the evidence and resolves the outcome on-chain. No funds move unless the contract says so.
...
<CardGroup cols={2}>
  ...
  <Card title="AI Agent Job Completion" icon="robot">
    Pay autonomous agents only when their on-chain SCA reports a task as complete, verified by an arbiter contract.
  </Card>
...
1. **Create** — The depositor locks USDC in `ArcFlareEscrow.sol`, naming a beneficiary and an arbiter.
...
3. **Dispute** — If either party calls `dispute`, funds are frozen and the arbiter reviews the case.
4. **Resolve** — The arbiter calls `resolveDispute` on-chain to send funds to either party.
...
curl -X POST https://flarehq.xyz/api/escrow/create \
  ...
  -d '{
    "depositorSCA": "0xYourSCAWalletAddress",
    "depositorWalletId": "circle-wallet-uuid-here",
...
```json
{
  ...
  "contractAddress": "0x...",
  "message": "200.00 USDC locked in ArcFlareEscrow contract on Arc Testnet. Both parties must confirm to release."
}
```
...
<ParamField body="depositorSCA" type="string" required>
  The Circle SCA wallet address of the party locking the funds. This wallet must hold sufficient USDC.
</ParamField>

<ParamField body="depositorWalletId" type="string" required>
  The Circle wallet UUID used to sign the on-chain transactions. Retrieve this from `GET /api/agent/status`.
</ParamField>

<ParamField body="beneficiarySCA" type="string" required>
  The SCA wallet address of the party that receives funds on release.
</ParamField>
...
curl -X POST https://flarehq.xyz/api/escrow/release \
  ...
  -d '{
    "reference": "escrow_m8k2p1_ab12cd",
    "callerSCA": "0xYourSCAWalletAddress"
  }'
```
...
Once a dispute is raised, the escrow status changes to `DISPUTED` and neither party can release funds unilaterally. The arbiter — `0x3500000000000000000000000000000000008004` by default, the ERC-8004 registry contract on Arc — reviews the case and calls `resolveDispute` on-chain.
...
The default arbiter address is `0x3500000000000000000000000000000000008004`, the ERC-8004 agent registry contract on Arc testnet. You can substitute any address — a multisig, a DAO, or a trusted third-party contract — by passing a different `arbiter` value when creating the escrow.
...
<Warning>
  **Automatic release at deadline.** If `deadlineHours` passes and no dispute has been raised, `ArcFlareEscrow.sol` releases funds automatically to the beneficiary. Raise a dispute **before** the deadline if you believe conditions have not been met — you cannot dispute an escrow once it has auto-released.
</Warning>
