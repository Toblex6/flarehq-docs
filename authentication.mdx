...
curl -X POST https://flarehq.xyz/api/payments/initialize \
  -H "Authorization: Bearer fhq_sec_test_4a8f2c..." \
...
|---|---|---|---|
| `401 Unauthorized` | `Authentication required` | No `Authorization` header was sent, or the key is malformed. | Check that your header is `Authorization: Bearer fhq_sec_...` with no extra characters. |
...
{
  "success": false,
  "error": "Authentication required. Provide a valid API key in the Authorization header."
}
```
