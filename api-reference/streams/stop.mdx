---
title: "POST /api/payments/stream/stop — Stop a Payment Stream"
sidebarTitle: "Stop Stream"
description: "POST /api/payments/stream/stop — halts an active USDC stream and returns the unstreamed balance to the sender's wallet."
---

Stopping a stream calls `stopStream(bytes32)` on the `ArcFlareStream` contract, which immediately halts all further accrual. The contract calculates how much USDC the receiver has earned up to that exact second, sends that amount to the receiver, and returns the remaining unstreamed balance to the sender. The stream record is updated to `STOPPED` and `stoppedAt` is recorded. Only the sender's SCA address may stop a stream — the contract enforces this at the chain level.
...
<ParamField body="callerSCA" type="string" required>
  The sender's SCA wallet address. This wallet signs the `stopStream` transaction. Must match the `senderSCA` of the stream.
</ParamField>
...
<ResponseField name="txHash" type="string">
  On-chain transaction hash for the `stopStream` call.
</ResponseField>
...
<Note>
  The receiver's accrued balance is automatically sent to them as part of the `stopStream` on-chain call — they do not need to call `/withdraw` separately after a stream is stopped.
</Note>
