---
title: "POST /api/escrow/create — Lock USDC On-Chain"
sidebarTitle: "Create Escrow"
description: "POST /api/escrow/create — locks USDC in ArcFlareEscrow.sol on Arc testnet. Returns reference, txHash, and explorer link."
---

When you create an escrow, FlareHQ performs two on-chain steps on your behalf: it first issues a USDC `approve` transaction so the contract can pull funds, then calls `createEscrow` on `ArcFlareEscrow.sol` to lock the amount. Both transactions are signed through Circle's Developer-Controlled Wallets SDK and confirmed on Arc Testnet (Chain ID 5042002) before the API returns. The response includes the `reference` ID you will use for every subsequent release, dispute, or status query.

<Note>
  The `depositorWalletId` is the Circle wallet UUID — **not** the SCA address. Retrieve it from `GET /api/agent/status` before calling this endpoint.
</Note>
...
<ParamField body="condition" type="string">
  A human-readable description of the release condition — e.g. `"Delivery of 500 API credits confirmed"`. Stored on-chain as a string parameter to `createEscrow`. Defaults to `"No condition set"` if omitted.
</ParamField>
...
<ResponseField name="escrow" type="object">
  ...
  <Expandable title="escrow fields">
    ...
    <ResponseField name="escrow.contractAddress" type="string">
      The `ArcFlareEscrow.sol` contract address that holds the funds.
    </ResponseField>
...
<ResponseField name="txHash" type="string">
  The on-chain transaction hash for the `createEscrow` call — e.g. `"0xabc123..."`.
</ResponseField>
...
```json
{
  ...
  "escrow": {
    ...
    "currency":        "USDC",
    "status":          "ACTIVE",
    "contractAddress": "0xArcFlareEscrowContractAddress",
    "depositorSCA":    "0xDepositorSCAWalletAddress",
    "beneficiarySCA":  "0xBeneficiarySCAWalletAddress",
    "condition":       "Delivery of 500 API credits confirmed",
  ...
  "message":     "200 USDC locked in ArcFlareEscrow contract on Arc Testnet. Both parties must confirm to release.",
  "nextSteps": {
...
```json
{
  ...
  "hint": "The depositor SCA wallet needs USDC. Fund at https://faucet.circle.com — select ARC-TESTNET."
}
```

<Warning>
  Both the `approve` and `createEscrow` on-chain steps can take up to 75 seconds to confirm. Do not retry the request during this window — doing so may result in duplicate escrows and double-spending from the depositor's wallet.
</Warning>
