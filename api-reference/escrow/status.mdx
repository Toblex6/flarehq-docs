---
title: "GET /api/escrow/status — Query Escrow State"
sidebarTitle: "Escrow Status"
description: "GET /api/escrow/status?reference= — returns current escrow status, amounts, parties, and on-chain transaction details."
---
...
<ResponseField name="escrow" type="object">
  ...
  <Expandable title="escrow fields">
    ...
    <ResponseField name="escrow.txHash" type="string">
      On-chain transaction hash for the original `createEscrow` call.
    </ResponseField>
    ...
    <ResponseField name="escrow.contractAddress" type="string">
      The `ArcFlareEscrow.sol` contract address holding the locked funds.
    </ResponseField>
    <ResponseField name="escrow.depositorConfirmed" type="boolean">
      Whether the depositor has submitted a `confirmDelivery` call.
    </ResponseField>
    <ResponseField name="escrow.beneficiaryConfirmed" type="boolean">
      Whether the beneficiary has submitted a `confirmDelivery` call.
    </ResponseField>
  </Expandable>
</ResponseField>
