...
<ParamField body="ownerNode" type="string">
  Ethereum address of the operator who controls this agent. Used to set the owner node in the ERC-8004 registry. Defaults to a FlareHQ-managed placeholder address if omitted — override this in production with your own address.
</ParamField>
...
Returned when Circle API credentials are missing from the FlareHQ server environment, or when on-chain registration is rejected.

```json
{
  "error": "Circle infrastructure variables missing from environment configurations."
}
```

### 408 — Timeout

Returned when the on-chain transaction does not confirm within ~50 seconds (20 polling attempts × 2.5 s).
...
Open the `explorerUrl` from the response on [ArcScan](https://testnet.arcscan.app) to confirm the `register()` transaction succeeded. The ERC-8004 Identity Registry contract is at `0x3500000000000000000000000000000000008004` on Arc testnet (chain ID `5042002`).
