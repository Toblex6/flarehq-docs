---
title: "Execute an x402 Micro-Payment — POST /api/x402/pay"
...
<Note>
  This endpoint does **not** require an `x-api-key` header. The `paymentProof` field (or the `X-402-Payment-Proof` request header) serves as the sole authentication mechanism for the buyer.
</Note>
