...
<ParamField body="webhookUrl" type="string">
  HTTPS URL that FlareHQ will `POST` to when the payment settles (e.g. `payment.settled`). The webhook body carries the settlement fields; poll [GET /api/payments/verify/{reference}](/api-reference/payments/verify) for the authoritative status. Must be publicly reachable.
</ParamField>
